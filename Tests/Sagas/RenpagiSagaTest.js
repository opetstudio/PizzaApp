...
import { getRenpagi } from '../../App/Sagas/RenpagiSagas'
import RenpagiActions from '../../App/Redux/RenpagiRedux'
...
test('first calls API', () => {
  const step = stepper(getRenpagi(FixtureAPI, { data: { page: 1 } }))
  // first yield is API
  expect(step()).toEqual(call(FixtureAPI.getRenpagi, { page: 1 }))
})
...
test('success path', async () => {
  ...
  const response = await api.getRenpagi({ newerModifiedon: 1494844278993 })
  // const response = FixtureAPI.getRestapi({ page: 1 })
  const step = stepper(getRenpagi(api, { newerModifiedon: 1494844278993 }))
  // first step API
  step()
  ...
  // const byId = dataApi
  expect(stepResponse).toEqual(put(RenpagiActions.renpagiSuccess(byId, allIds, maxModifiedon)))
})
